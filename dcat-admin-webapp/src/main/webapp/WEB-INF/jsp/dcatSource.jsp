...
<body>
				...
				<div class="col-md-12">
					<h3>
						Harvest History (100 last harvests) - <a target="_blank"
							href="
                <%out.print(pageContext.getRequest().getServerName().split(":")[0] + ":");%>${kibanaLink.getPort()}${kibanaLink.getFirstHalf()}${dcatSource.getIdUrlEncoded()}${kibanaLink.getSecondHalf()}">
					...
					<table class="table table-striped">
						...
						<tbody>
							<c:forEach var="harvest"
								items="${dcatSource.getHarvestedLast100()}">
								<tr>
	...
	<script type="text/javascript">
		var saveDcatSource = function() {
			...
			sendRequest('POST',
					'${pageContext.request.contextPath}/api/admin/dcat-source',
					data);
		};

		var deleteDcatSource = function(dcatSourceId) {
			sendRequest('DELETE',
					'${pageContext.request.contextPath}/api/admin/dcat-source?delete='
							+ dcatSourceId, null);
		};
